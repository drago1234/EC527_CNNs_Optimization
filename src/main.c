...
Run this first, it will produce a gmon.out file, which will be read by gprof.
$ ./MLP 3 4,5,5 softmax,relu,tanh 1 sigmoid 0.01 10000 data/data_train.csv 1096 5 data/data_test.csv 275 5 
Then use gprof to interpret gmon.out:
$ gprof ./MLP gmon.out > run1.withO1_pg.stats
...
int main(int argc, char** argv) {
    ...
    */
    ...
    // L2 Regularization parameter,  Learning rate parameter Ex: 0.01
    ...
    // Total number of layers: n_hidden + 1(input) + 1(output)
    int n_layers = param->n_hidden + 2;

    // Save the sizes of layers in an array: calloc() gives you a zero-initialized buffer, while malloc() leaves the memory uninitialized.
    int* layer_sizes = (int*)calloc(n_layers, sizeof(int));

    // Initialize the layer size for input and output
    layer_sizes[0] = param->feature_size - 1;
    layer_sizes[n_layers-1] = param->output_layer_size;

    // Initialized the layer size for each hidden layer
    ...
    printf("Training:\n");
    printf("---------\n");
    mlp_trainer(param, layer_sizes);    // layer_sizes=5 in our case
    printf("\nDone.\n\n");
