...
__global__ void cuda_single_block_SOR2(int rowlen, float *data, double change);
...
int main(int argc, char *argv[])
{
  ...
  if (work_on_part_2){    // Running code for lab7 part 2
    ...
    cuda_single_block_SOR2<<<1, dimBlock>>>(arrLen, d_data, change);   // 2D interleaved block
  }else{
    int iters = 0;
...
/* 
The idea for this implementation is similar to the previous one. However, instead of asking each thread to handle a block, we are doing this in a interleaved fashion, where the number of element in a block is the number of threads available, and the step size for each thread is blockDim.
*/
__global__ void cuda_single_block_SOR2(int rowlen, float *data, double change){
  // cuPrintf("Hello from cuda_single_block_SOR2!\n");
  long int i, j, iters;
  // Who I am??
  for(iters=0; iters < ITERATIONS; iters++){
    for (i = threadIdx.x; i <= rowlen-1; i += blockDim.x) {
      for (j = threadIdx.y; j <= rowlen-1; j += blockDim.y) {
        if (i>=1 && i<rowlen-1 && j>=1 && j<rowlen-1){  // skip the boundary element
          change = data[i*rowlen+j] - .25 * (data[(i-1)*rowlen+j] +
                                    data[(i+1)*rowlen+j] +
                                    data[i*rowlen+j+1] +
                                    data[i*rowlen+j-1]);
          __syncthreads();    // Must SYNC between read and writes.
          data[i*rowlen+j] -= change * OMEGA;
          // cuPrintf("data: %f\n", data[i*rowlen+j]);
          // cuPrintf("[i, j]: [%d, %d]\n", i, j);
          __syncthreads();
...
// This version is still problematic --> Don't use it!!!
 ...
 */
__global__ void cuda_SOR(int rowlen, float *data, double change){
  long int i, j;
  // i = threadIdx.x + blockIdx.x * blockDim.x;
  // j = threadIdx.y + blockIdx.y * blockDim.y;
