...
void calculate_local_gradient(parameters* param, int layer_no, int n_layers, int* layer_sizes, double** layer_inputs, double** layer_outputs,
    double* expected_output, double** local_gradient, double*** weight_cal) {
            ...
            case 1: // identity
                ...
                for (i = 0; i < layer_sizes[layer_no]; i++) {
                    ...
                    for (j = 0; j < layer_sizes[layer_no+1]; j++)
                        error += local_gradient[layer_no+1][j] * weight_cal[layer_no][i][j];
...
void back_propagation(parameters* param, int training_example, int n_layers, int* layer_sizes, double** layer_inputs, double** layer_outputs, double*** weight_back, int id) {
    /* Set time variable*/
    ...
    calculate_local_gradient(param, n_layers-1, n_layers, layer_sizes, layer_inputs, layer_outputs, expected_output, local_gradient, weight_back);
    ...
    for (i = 0; i < n_layers-1; i++) {
        for (j = 0; j < layer_sizes[i]+1; j++) {
            for (k = 0; k < layer_sizes[i+1]; k++) {
                weight_back[i][j][k] -= weight_correction[i][j][k];
            }
        }
    }
